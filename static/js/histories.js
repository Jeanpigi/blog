document.addEventListener("DOMContentLoaded", async () => {
  const container = document.getElementById("container-historias");
  if (!container) return;
  ...
  try {
    ...
    container.innerHTML = `
      <div style="text-align:center;padding:4rem 0;color:#ef4444;font-size:1.5rem">
        No pudimos cargar las historias. Intenta de nuevo.
      </div>`;
  } finally {
    Swal.close();
  }
  ...
  function buildCard(post) {
    ...
    article.innerHTML = `
      <header class="post-head">
        <div class="post-date">
          <span class="post-categoria-badge historias">Historias</span>
